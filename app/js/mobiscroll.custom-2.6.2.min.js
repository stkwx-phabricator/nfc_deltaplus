//added by Warren.Si
function rePosition(){
//	var leftPx = ($('body').width()/2-$('.dw').width()*baseCount/2)/baseCount,
//	topPx = parseInt($('.dwbg').css('top').replace('px',''));
//	$('.dwbg').css('zoom',baseCount).css('left',leftPx+'px').css('top',topPx/(baseCount)+'px');
}
...
n.alert(p.ariaDesc)}a(".dwwl",v).on("DOMMouseScroll mousewheel",Ja).on(C,Fa).on("keydown",Ha).on("keyup",Ia);v.on(C,".dwb-e",Ga).on("keydown",".dwb-e",function(c){if(c.keyCode==32){c.preventDefault();c.stopPropagation();a(this).click()}});E("onShow",[v,R]);/*added by warren.si*/rePosition()};n.hide=function(c,b){if(!W||!1===E("onClose",[R,b]))return!1;a(".dwtd").each(function(){a(this).prop("disabled",!1).removeClass("dwtd");a(this).data("autocomplete")?a(this).attr("autocomplete",a(this).data("autocomplete")):a(this).removeAttr("autocomplete")});
